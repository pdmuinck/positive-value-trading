...
var goldenvegasWS = new WebSocket("wss://wss.goldenvegas.be");
goldenvegasWS.on('open', function open() {
  goldenvegasWS.send(JSON.stringify({
    "Id": "ad92eff2-7e71-0626-5917-1f5ac121cd8a",
    "TTL": 10,
    "MessageType": 1,
    "Message": "{\"NodeType\":1,\"Identity\":\"2251ebbf-e153-4b49-ac5d-2477d1ac7644\",\"EncryptionKey\":\"\",\"ClientInformations\":{\"AppName\":\"Front;Registration-Origin: default\",\"ClientType\":\"Responsive\",\"Version\":\"1.0.0\",\"UserAgent\":\"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.66 Safari/537.36\",\"LanguageCode\":\"nl\",\"RoomDomainName\":\"GOLDENVEGAS\"}}"
  })); // football request

  goldenvegasWS.send(JSON.stringify(goldenvegasWS.send(JSON.stringify({
...
goldenvegasWS.on('message', function incoming(data) {
  var response = JSON.parse(JSON.parse(data).Message);

  if (response.Requests) {
    var test = JSON.parse(response.Requests[0].Content);
    var events = test.LeagueDataSource.LeagueItems.map(function (league) {
...
goldenvegas.getEventsForBookAndSport = function _callee(sport) {
  return regeneratorRuntime.async(function _callee$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          return _context.abrupt("return", cache.get('FOOTBALL'));
