...
circusWS.on('message', function incoming(data) {
    ...
    if(JSON.parse(bla.Message)["$type"] === 'APR.Packets.DataPacket, APR.Packets') {
        ...
        cache.set('EVENTS', JSON.parse(JSON.parse(bla.Message).Requests["$values"][0].Content).LeagueDataSource.LeagueItems.map(league => league.EventItems).flat().map(event => {return {id: event.EventId, participants: [{id: event.Team1Name, name: event.Team1Name}, {id: event.Team2Name, name: event.Team2Name}]}}))
    }
})
