...
module.exports = function(server) {
    ...
    server.get('/bookmakers/:book/events', async (req, resp) => {
        ...
        .then(response => resp.send({provider: 'PINNACLE', book: req.params.book, events: response.flat()}))
    ...
    server.get('/bookmakers/:book/events/:eventId/betoffers', async (req, resp) => {
        ...
        .then(response => resp.send({provider: 'PINNACLE', book: req.params.book, eventId: req.params.eventId, data: response}))
        .catch(error => resp.status(404).send(error.message))
    })
