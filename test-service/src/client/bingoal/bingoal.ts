        ...
        return axios.get(leagueUrl, headers)
            .then(response => {
                return response.data.sports.map(sport => sport.matches).flat().filter(match => !match.outright).map(match => {
                    ...
                    return new EventInfo(match.betradarID, getSportRadarEventUrl(match.betradarID), [bookmakerInfo])
                })
            })})
}
    ...
    event.importantSubbets.forEach(subbet => {
        ...
        if(betType !== BetType.UNKNOWN) {
            subbet.tips.forEach(tip => {
                betOffers.push(new BetOffer(betType, event.ID, Bookmaker.BINGOAL, tip.shortName, parseFloat(tip.odd), undefined))
            })
        }
    })
    return betOffers
